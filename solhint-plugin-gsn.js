    ...
    ContractDefinition(node) {
        ...
        if (node.baseContracts.length === 0) {
            ...
            if (!this.foundGsnImport) {
                this.reporter.error(node, this.ruleId, `Context.sol is not imported in a non-inherited GSN compatible contract`, fixer => {
                    return fixer.insertTextAfterRange(this.lastImportDirective != undefined ? this.lastImportDirective.range : this.pragmaDirective.range, `;\n\nimport "${this.gsnImportPath}";`)
                })
            }
            
            this.reporter.error(node, this.ruleId, `Context.sol is not inherited in a non-inherited GSN compatible contract`, fixer => {
        ...
        if (node.expression.name === 'msg' && (mName === 'data', mName === 'sender')) {
            this.reporter.error(node, this.ruleId, `Avoid to use of msg.${mName} when writing GSN compatible contracts`, fixer => {
                if (this.inheritance && !this.foundGsnImport && !this.inheritanceWarning) {
                    ...
                    console.log(`GSN solhint plugin fixed ${this.contractName} but no GSN import was present. This could cause compile errors if ${this.contractName} doesn't inherit from Context.sol.`)
                }
